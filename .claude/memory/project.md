...
- **A/C eco mode only sticks in Cool mode** (found 2026-07-29) — setting `energySavingStatus=1` in any mode but Cool silently reverts to off within ~60s on this unit; not an app bug, a device constraint. `device_card.html` now disables the Eco button (with an explanatory tooltip) unless `ac_mode == "cool"`.
